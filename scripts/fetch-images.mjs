...
// id -> Commons file title. Chosen by hand from the contact sheets.
const SOURCES = {
  ...
  "anglepoise": "File:Type 1227 desk lamp, Anglepoise, designed by George Carwardine, manufactured by Herbert Terry and Sons, 1935 - Design Museum, Kensington - London - DSC01571.jpg",
  ...
  "lego-patent": "File:US3005282A Toy building brick (1958 filed, 1961 published) by Christiansen Godtfred Kirk - Lego brick, p.2, Fig. 7~12.png",
};
...
const titles = Object.values(SOURCES);
const info = {};
for (let i = 0; i < titles.length; i += 40) {
  const u = new URL("https://commons.wikimedia.org/w/api.php");
  u.search = new URLSearchParams({
    action: "query", titles: titles.slice(i, i + 40).join("|"), prop: "imageinfo",
    iiprop: "url|size|mime|extmetadata", iiurlwidth: String(WIDTH), format: "json",
  });
...
for (const [id, title] of Object.entries(SOURCES)) {
  const p = info[title];
  if (!p || p.missing !== undefined || !p.imageinfo) { console.log("MISSING", id, title); continue; }
  const ii = p.imageinfo[0];
  const em = ii.extmetadata || {};
  const thumb = ii.thumburl || ii.url;
  const ext = (ii.thumbmime || ii.mime || "image/jpeg").includes("png") ? "png" : "jpg";
